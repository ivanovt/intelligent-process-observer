...
not add a Jira Track and Release endpoint, API field mapping, transport, authentication,
credentials, retry/backoff policy, pagination/truncation policy, production model or
provider selection, provider-specific model settings, or external live calls. The
existing PydanticAI dependency, where used, SHALL remain in an infrastructure adapter
behind the framework-neutral agent boundary.
