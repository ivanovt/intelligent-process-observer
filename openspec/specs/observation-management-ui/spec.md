...
The system SHALL present Observation management under the existing `Observations` navigation area using the frozen UI Direction v1.1 shell, hierarchy, terminology, and restrained engineering-dashboard visual language. It SHALL provide routes for the definitions list, read-only definition inspection, aggregate creation, and nested Metric Lens, Alert Lens, and Relationship editors without exposing a separate Admin area.
