...
| Active assignment | VS-04-C02 bounded correction |
| Last accepted slice | VS-03 |
| Bounded correction | VS-04-C02 — add the remaining exact two-Relationship conditional/always-applicable payload, browser-back non-mutation, duplicate assignment rejection, and rendered frame-11 validation-accessibility evidence. Test-only unless evidence exposes an in-scope defect; expected risk: normal. |
