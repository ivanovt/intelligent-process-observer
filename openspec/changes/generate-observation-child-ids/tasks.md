...
- [ ] 4.1 Update `docs/ui/README.md`, `docs/ui/ui_implementation_handoff_v1.md`, `docs/ui/frontend_ui_stack_adr.md`, and the current UI-direction guidance in `AGENTS.md` to identify UI Direction v1.3 and document generated, read-only, stable Metric Lens, Alert Lens, and Relationship IDs; preserve the accepted frontend stack, UI authority order, historical v1.1/v1.2 statements, Data Sources behavior, and all existing semantic boundaries.

## 5. Verification

- [ ] 5.1 Run focused frontend tests for the generator, all three nested editors, and the aggregate create flow after current-main reconciliation, including 255-character and collision-after-truncation boundaries; resolve failures within the approved behavior.
- [ ] 5.2 Run `make check` as the final local verification and report any failure accurately before re-archive and pull-request update work.
