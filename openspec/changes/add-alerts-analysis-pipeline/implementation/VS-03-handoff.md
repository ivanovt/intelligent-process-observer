...
Verification: `cd backend && uv run pytest tests/test_alert_contracts.py tests/test_alert_analysis_pipeline.py -q` (20 passed); `cd backend && uv run ruff check src/app/alerts tests/test_alert_analysis_pipeline.py tests/test_runtime_persistence_integration.py`; and `cd backend && uv run ruff format --check src/app/alerts tests/test_alert_analysis_pipeline.py tests/test_runtime_persistence_integration.py` (all passed). `test_invalid_current_subset_persists_correlated_partial` was collected and skipped because `IPO_TEST_DATABASE_URL` was unset; no local Compose service was available.
