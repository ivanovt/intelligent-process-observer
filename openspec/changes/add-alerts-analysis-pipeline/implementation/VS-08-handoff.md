...
| VS08-AC05 | `test_optional_timeout_continues_to_valid_completion` | Timed-out optional execution remains non-terminal and the next completion succeeds. |
...
- PostgreSQL adapter-terminal proof — 1 passed (one existing Alembic configuration warning).
...
Known limitation: optional-timeout continuation is adapter-tested; its dedicated
PostgreSQL proof remains a VS-09 whole-change follow-up.
