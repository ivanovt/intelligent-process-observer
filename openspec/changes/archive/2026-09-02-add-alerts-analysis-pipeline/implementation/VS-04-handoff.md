...
to artifact-free failed terminal outcomes. All four current-acquisition fixtures use a
fail-on-call analyzer, agent, and builder to prove that acquisition failures stop before
later phases. The deterministic-analysis PostgreSQL fixture supplies a canonical typed,
strictly in-window record and proves the injected analyzer was called before its forced
failure. The persistence seam advances a failed LensRun without inserting an Alert
result; usable outcomes retain their existing artifact path.
...
|---|---|---|---|---|---|
| VS04-AC01 | service; PostgreSQL collected | `tests/test_alert_analysis_pipeline.py::test_current_acquisition_failures_map_exact_reason_and_stop`; `tests/test_runtime_persistence_integration.py::test_current_acquisition_failures_persist_exact_reason_without_artifact` | Typed/thrown error maps `current_query_failed`; typed/thrown timeout maps `current_query_timeout`; the durable reason has `component: null`; no artifact. | Fail-on-call analyzer, agent, and builder for every fixture; distinct fixtures prevent generic mapping. | service PASS; PostgreSQL SKIPPED (URL unset) |
...
| VS04-AC03 | service; PostgreSQL collected | `tests/test_alert_analysis_pipeline.py::test_mandatory_analysis_failure_stops_agent_and_builder`; `tests/test_runtime_persistence_integration.py::test_mandatory_analysis_failure_is_terminal_without_artifact` | A typed canonical record at 00:30Z lies strictly within the 00:00Z–01:00Z window; the injected analyzer is called exactly once, then its error maps `deterministic_analysis_failed` with durable `component: null`; no agent, builder, or artifact. | Analyzer-call assertion; fail-on-call agent/builder. | service PASS; PostgreSQL correction rerun SKIPPED (no URL/service) |
...
artifact only for usable outcomes. The established runtime repository payload retains an
absent optional reason component as JSON `null`. Provider, analysis, agent, and
persistence errors are kept distinct; infrastructure errors are not remapped.

## Plan guidance deviations

| Deviation | Evidence / rationale | Coordinator disposition |
|---|---|---|
| none | The focused correction adds the required AC01 analyzer-bypass guard and aligns VS-04 PostgreSQL assertions with the established repository reason payload contract; both are inside the approved test scope. | PENDING |
...
- `cd backend && uv run pytest tests/test_runtime_persistence_integration.py::test_mandatory_analysis_failure_is_terminal_without_artifact -q` — collected and skipped: no local PostgreSQL service or `IPO_TEST_DATABASE_URL`.
