...
Status: both accepted MEDIUM findings resolved by independent Sol verification; ready for archive.
...
Sol finding verification: IR-001 `RESOLVED` (54 focused reporting tests passed); SR-001 `RESOLVED` (2 focused projector tests passed). Neither verifier found a fix-adjacent regression. Neither reran the full check independently; the Coordinator's post-fix full-check result above remains the repository-wide verification signal.
