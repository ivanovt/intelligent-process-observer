...
- [x] 6.1 Remove lexical semantic and partial Markdown-syntax blacklists from runtime validation; retain strict non-blank and exact source-membership validation, and make the deterministic renderer normalize line breaks and escape every model-authored or source-authored value so only renderer constants can create Markdown structure.
- [x] 6.2 Render the correlated Observation and run identifiers with deterministic English labels, and stop copying raw Observation name, description, or analytical-objective text into the final Markdown when no declared English context-presentation field exists.
- [x] 6.3 Replace blacklist-oriented tests with focused coverage for escaped Markdown/control input, harmless negated boundary language, structurally undeclared fields and items, non-English semantic context exclusion, renderer-owned headings, and agent-instruction/evaluation examples for recommendations, causal overstatement, certainty preservation, and English presentation.
- [x] 6.4 Run focused reporting tests and Ruff checks, run `make check`, and re-run bounded finding verification plus the repository implementation review before archive consideration.

Completion record: the two accepted `MEDIUM` findings recorded at `c3b55d7` are resolved. IR-001 now preserves opaque traceability identity injectively across types, controls, escape-looking text, delimiters, locator boundaries, astral scalars, surrogate code units, and arbitrary nonnegative indexes. IR-002 now excludes raw semantic context from presentation and strengthens adversarial evaluation for direction, grounding, causal invention, advice paraphrases, and context leakage. Focused Ruff and reporting checks passed (`64 passed, 1 skipped`); `make check` passed (`742 passed, 58 skipped`), including frontend lint/build and strict OpenSpec validation. The skipped live-model evaluation remains an explicit opt-in, non-blocking evaluation. A database-enabled rerun was not applicable to this correction delta because it changes only presentation encoding, agent instructions, and tests and does not touch persistence; no dedicated `IPO_TEST_DATABASE_URL` was configured, so development data was not used. Bounded finding verification resolved IR-001 and IR-002 with no fix-adjacent regression, and the fresh GPT-5.6 Sol whole-change implementation review returned `READY` with no bounded correction or structural/normative escalation.
