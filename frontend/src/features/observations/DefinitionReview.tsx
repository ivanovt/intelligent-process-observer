...
type ReviewDefinition=Pick<ObservationDraft,'name'|'description'|'objective'|'lenses'|'alert_lenses'|'relationships'>

/** Renders a read-only, ordered configuration review without changing the draft. */
export function DefinitionReview({definition,liveErrors={},explicitErrors={}}:{definition:ReviewDefinition;liveErrors?:DraftErrors;explicitErrors?:DraftErrors}) {
  ...
  const status=explicitCount?{icon:<CircleAlert size={16} aria-hidden="true"/>,text:`${explicitCount} submitted issue${explicitCount===1?'':'s'} remain`,tone:'text-[var(--color-error)]'}:appearsComplete?{icon:<CheckCircle2 size={16} aria-hidden="true"/>,text:'Ready to create',tone:'text-[var(--color-success)]'}:{icon:<TriangleAlert size={16} aria-hidden="true"/>,text:'Complete required details',tone:'text-[var(--color-warning)]'}
  return <section aria-labelledby="review-heading" className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
    <div className="flex flex-wrap items-baseline justify-between gap-2"><div><h2 id="review-heading">Review</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">Verify the aggregate that will be submitted with Create Observation.</p></div><p className={`flex items-center gap-1 text-sm font-medium ${status.tone}`}>{status.icon}{status.text}</p></div>
    {explicitCount?<p className="mt-3 text-sm text-[var(--color-text-secondary)]">{appearsComplete?'Values appear complete. Create again to confirm them.':'Correct the highlighted values, then create again to revalidate.'}</p>:remaining.length?<p className="mt-3 text-sm text-[var(--color-text-secondary)]">Complete the required details before creating this Observation.</p>:<p className="mt-3 text-sm text-[var(--color-text-secondary)]">All currently required definition fields are complete.</p>}
