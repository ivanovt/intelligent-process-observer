...
export function KnowledgeScopeField({ draft, onChange }: { draft: ObservationDraft; onChange: (scope: KnowledgeScope | null) => void }) {
  ...
  const setScope = (service_ids: string[], service_version = version) => onChange(service_ids.length ? { service_ids, service_version: service_version.trim() || null } : null)
  ...
  return <section id="knowledge-scope" className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">Knowledge scope</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">Optional retrieval context for approved knowledge. It is not Observation evidence or an execution setting.</p></div><Button type="button" variant="secondary" disabled={pending} onClick={requestSuggestion}><Sparkles size={17} aria-hidden="true" />{pending ? 'Suggesting…' : 'Suggest knowledge scope'}</Button></div><div className="mt-5 grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]"><Field label="Service ID" description="Add canonical service IDs explicitly."><Input value={serviceId} onChange={event => setServiceId(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); add() } }} placeholder="mprm-server" /></Field><Button type="button" variant="secondary" className="self-end" onClick={add}>Add service</Button></div>{scope?.service_ids.length ? <ul aria-label="Selected knowledge services" className="mt-3 flex flex-wrap gap-2">{scope.service_ids.map(id => <li key={id} className="flex items-center gap-1 rounded-full border border-[var(--color-warning-border)] bg-[var(--color-warning-surface)] px-2.5 py-1 text-sm text-[var(--color-warning)]">{id}<button type="button" aria-label={`Remove ${id}`} onClick={() => setScope(scope.service_ids.filter(item => item !== id))}><X size={15} aria-hidden="true" /></button></li>)}</ul> : <p className="mt-3 text-sm text-[var(--color-text-secondary)]">No knowledge scope is set. Only globally applicable knowledge can be retrieved.</p>}<div className="mt-4 max-w-md"><Field label="Service version (optional)" description="An opaque service-version label applied to this retrieval context."><Input value={version} onChange={event => updateVersion(event.target.value)} placeholder="2.x" disabled={!scope?.service_ids.length} /></Field></div>{suggestion ? <div className="mt-4 rounded-lg border border-[var(--color-info-border)] bg-[var(--color-info-surface)] p-4"><p className="text-sm text-[var(--color-info)]">Suggested knowledge scope: {suggestion.join(', ')}</p><div className="mt-3 flex gap-2"><Button type="button" onClick={accept}>Accept suggestion</Button><Button type="button" variant="secondary" onClick={() => setSuggestion(null)}>Dismiss</Button></div></div> : null}{notice ? <div className="mt-4"><InlineNotice tone="info">{notice}</InlineNotice></div> : null}</section>
}
