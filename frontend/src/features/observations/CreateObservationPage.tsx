import { Activity, BellRing, CheckCircle2, CircleAlert, GitFork, Pencil, Plus, SlidersHorizontal, TriangleAlert } from 'lucide-react'
...
export function CreateObservationPage() {
  const { draft, explicitErrors: errors, fresh, clear, setExplicitErrors, updateGeneral } = useObservationDraft()
  ...
  const submit = async () => {
    ...
    try {
      ...
      clear()
      navigate(`/observations/${created.id}`, { replace: true, state: { created: true } })
    } catch (error) {
      const api = error instanceof ApiError ? error : null
      if (api?.field) setExplicitErrors({ [api.field]: api.message })
      else setFailure(api?.message ?? 'Unable to create the Observation definition.')
  ...
  return <section className="mx-auto max-w-6xl"><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
    <PageHeader className="mb-3" eyebrow="Observations / Create" title="Create Observation" description="Configure one validated Observation Definition." />
    <FormSideRail className="lg:col-start-2 lg:row-span-2 lg:row-start-1" actions={<><Button variant="secondary" disabled={pending} onClick={() => { clear(); navigate('/observations') }}>Cancel</Button><Button onClick={submit} disabled={pending}><CheckCircle2 size={17} aria-hidden="true" />{pending ? 'Creating…' : 'Create Observation'}</Button></>}>
      <DefinitionSummary metricCount={draft.lenses.length} alertCount={draft.alert_lenses.length} relationshipCount={draft.relationships.length} explicitIssueCount={issues.length} hasLens={hasLens} isReady={!Object.keys(liveErrors).length} />
    </FormSideRail>
    <div className="min-w-0">
      {location.state?.draftLost ? <InlineNotice tone="info">Your previous draft is unavailable. Start with a new Observation draft.</InlineNotice> : null}
      {failure ? <div className="mb-5"><InlineNotice tone="error">{failure}</InlineNotice></div> : null}
      {issues.length ? <div className="mb-5"><ValidationSummary ref={summaryRef} issues={issues} title="Observation cannot be created" /></div> : null}
