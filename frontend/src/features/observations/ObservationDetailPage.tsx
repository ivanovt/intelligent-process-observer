...
export function ObservationDetailPage() {
  ...
  return <DetailFrame>{confirmation}<PageHeader eyebrow="Observation definition" title={observation.name} description={observation.description?<span className="whitespace-pre-wrap">{observation.description}</span>:undefined} actions={<span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-1.5 text-xs font-medium text-[var(--color-text-secondary)] shadow-xs"><LockKeyhole size={14} aria-hidden="true"/>Read-only</span>}/><DefinitionInspection definition={observation}/></DetailFrame>
}
