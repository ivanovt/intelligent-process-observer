...
describe('KnowledgeScopeField',()=>{
  ...
  it('rejects a failed suggestion without changing the local draft',async()=>{const user=userEvent.setup();vi.stubGlobal('fetch',vi.fn().mockResolvedValue(new Response('{}',{status:503})));render(<Field/>);await user.click(screen.getByRole('button',{name:'Suggest knowledge scope'}));expect(await screen.findByText(/No knowledge scope suggestion is available/)).toBeTruthy();expect(screen.queryByLabelText('Selected knowledge services')).toBeNull()})
  it('discards a response made stale by an explicit scope edit',async()=>{const user=userEvent.setup();let resolve:(value:Response)=>void=()=>{};vi.stubGlobal('fetch',vi.fn().mockImplementation(()=>new Promise<Response>(done=>{resolve=done})));render(<Field/>);await user.click(screen.getByRole('button',{name:'Suggest knowledge scope'}));await user.type(screen.getByLabelText('Service ID'),'manual');await user.click(screen.getByRole('button',{name:'Add service'}));resolve(new Response(JSON.stringify({service_ids:['mprm-server']})));await new Promise(done=>setTimeout(done,0));expect(screen.queryByText(/Suggested knowledge scope/)).toBeNull();expect(screen.getByLabelText('Selected knowledge services').textContent).toContain('manual')})
})
