...
const metricCapabilities={metric:[{adapter_type:'prometheus',sources:[{id:'primary',name:'Primary Prometheus'}]}]}
...
async function createMetric(user:ReturnType<typeof userEvent.setup>, values={id:'cpu',name:'CPU utilization'}) {await user.click(screen.getByText('Add Metric Lens'));await screen.findByRole('option',{name:'Primary Prometheus'});await user.type(screen.getByLabelText('Lens ID'),values.id);await user.type(screen.getByLabelText('Name'),values.name);await user.type(screen.getByLabelText('Metric ID'),'node_cpu');await user.type(screen.getByLabelText('Unit'),'%');await user.selectOptions(screen.getByLabelText('Metric source'),'primary');await user.type(screen.getByLabelText('Provider query'),'rate_cpu_5m');await user.click(screen.getByLabelText('spike'));await user.click(screen.getByLabelText('drift'));await user.click(screen.getByText('Add reference period'));await user.type(screen.getByLabelText('Reference period 1'),'1d');await user.click(screen.getByText('Add reference period'));await user.type(screen.getByLabelText('Reference period 2'),'7d');await user.click(screen.getByText('Apply changes'))}
...
describe('Observation create routes',()=>{
  ...
  it('posts one exact Metric-only aggregate, retains its draft after a metric failure, and succeeds on retry',async()=>{const user=userEvent.setup();let postCount=0;const fetchMock=vi.fn().mockImplementation((url:string,init?:RequestInit)=>{if(url==='/api/v1/observation-definition-capabilities')return Promise.resolve(response(metricCapabilities));if(url==='/api/v1/observations'&&init?.method==='POST'){postCount+=1;return Promise.resolve(postCount===1?response({code:'validation_error',message:'Correct Metric query',field:'lenses.0.query'},422):response(metricCreated,201))}return Promise.resolve(response(metricCreated))});vi.stubGlobal('fetch',fetchMock);renderAt('/observations/new');await user.type(await screen.findByLabelText('Name'),'Metric health');await user.type(screen.getByLabelText('Objective'),'Observe metrics');await createMetric(user)
    ...
    const posts=fetchMock.mock.calls.filter(([url,init])=>url==='/api/v1/observations'&&(init as RequestInit).method==='POST');expect(posts).toHaveLength(2);expect(JSON.parse(String((posts[0][1] as RequestInit).body))).toEqual({name:'Metric health',description:null,objective:'Observe metrics',lenses:[{id:'cpu',name:'CPU utilization',description:null,type:'metric',metric_id:'node_cpu',adapter_type:'prometheus',source_id:'primary',query:'rate_cpu_5m',unit:'%',analysis_objectives:['spike','drift'],reference_periods:['1d','7d']}],alert_lenses:[],relationships:[]});expect(JSON.stringify((posts[0][1] as RequestInit).body)).not.toMatch(/clientKey|history|preflight|tool|free-text/i);expect(fetchMock.mock.calls.filter(([url])=>String(url).includes('preflight')||String(url).includes('/lenses/'))).toEqual([])
  })
  ...
  it('posts a relationship-free mixed aggregate with the same type-local Lens ID and no child transport',async()=>{const user=userEvent.setup();const mixed:ObservationResponse={...metricCreated,name:'Mixed health',lenses:[{...metricCreated.lenses[0],id:'shared'}],alert_lenses:[{...created.alert_lenses[0],id:'shared'}]};const fetchMock=vi.fn().mockImplementation((url:string,init?:RequestInit)=>url==='/api/v1/observation-definition-capabilities'?Promise.resolve(response(metricCapabilities)):url==='/api/v1/observations'&&init?.method==='POST'?Promise.resolve(response(mixed,201)):Promise.resolve(response(mixed)));vi.stubGlobal('fetch',fetchMock);renderAt('/observations/new');await user.type(await screen.findByLabelText('Name'),'Mixed health');await user.type(screen.getByLabelText('Objective'),'Observe mixed');await createMetric(user,{id:'shared',name:'Shared metric'});await createAlert(user,{id:'shared',name:'Shared alert',query:'project=MIXED'});await user.click(screen.getAllByRole('button',{name:'Create Observation'})[0]);expect(await screen.findByText(/created successfully/i)).toBeTruthy();const posts=fetchMock.mock.calls.filter(([url,init])=>url==='/api/v1/observations'&&(init as RequestInit).method==='POST');expect(posts).toHaveLength(1);const body=JSON.parse(String((posts[0][1] as RequestInit).body));expect(body).toEqual({name:'Mixed health',description:null,objective:'Observe mixed',lenses:[{id:'shared',name:'Shared metric',description:null,type:'metric',metric_id:'node_cpu',adapter_type:'prometheus',source_id:'primary',query:'rate_cpu_5m',unit:'%',analysis_objectives:['spike','drift'],reference_periods:['1d','7d']}],alert_lenses:[{id:'shared',name:'Shared alert',description:null,type:'alert',source:'jira_track_and_release',selector:{query:'project=MIXED'},analysis_objectives:[],reference_periods:[]}],relationships:[]});expect(JSON.stringify(body)).not.toMatch(/clientKey|history|preflight|tool|free-text/i);expect(fetchMock.mock.calls.map(([url,init])=>({url,method:(init as RequestInit | undefined)?.method??'GET'}))).toEqual([{url:'/api/v1/observation-definition-capabilities',method:'GET'},{url:'/api/v1/observations',method:'POST'},{url:`/api/v1/observations/${mixed.id}`,method:'GET'}])})
})
