...
const created={id:'created',name:'Release health',description:null,objective:'Observe releases',schema_version:1,lenses:[],alert_lenses:[],relationships:[],href:'/api/v1/observations/created'}
...
describe('Observation create routes',()=>{
  ...
  it('keeps the route-scoped draft through Alert edits, replaces by opaque key in order, and applies with zero writes',async()=>{
    ...
    const editLinks=screen.getAllByText('Edit');await user.click(editLinks[0]);await user.clear(screen.getByLabelText('Name'));await user.type(screen.getByLabelText('Name'),'Updated first');await user.click(screen.getByText('Apply changes'))
    expect((screen.getByText('Updated first').parentElement?.textContent ?? '')).toContain('Updated first')
  ...
  it('shows mapped and fallback server errors, retains values, provides child correction, and retries successfully',async()=>{
    const user=userEvent.setup();const errors=[{field:'alert_lenses.0',message:'Aggregate child error'},{field:'alert_lenses.0.selector.query',message:'Nested child error'},{field:'alert_lenses.99.name',message:'Out of range'},{field:'unknown.field',message:'Unknown field'},{message:'Aggregate failure'}];const fetchMock=vi.fn();for(const error of errors)fetchMock.mockResolvedValueOnce(response({code:'validation_error',...error},422));fetchMock.mockResolvedValueOnce(response(created,201)).mockResolvedValueOnce(response(created));vi.stubGlobal('fetch',fetchMock);renderAt('/observations/new');await populateValidDraft(user)
    for(const error of errors){await user.click(screen.getAllByRole('button',{name:'Create Observation'})[0]);expect((await screen.findByRole('alert')).textContent).toContain(error.message);expect((screen.getByLabelText('Name') as HTMLInputElement).value).toBe('Release health');if(error.field?.startsWith('alert_lenses.0'))expect(screen.getByText('Correct Alert Lens')).toBeTruthy()}
    await user.click(screen.getAllByRole('button',{name:'Create Observation'})[0]);expect(await screen.findByText(/created successfully/i)).toBeTruthy();expect(fetchMock).toHaveBeenCalledTimes(7)
  })
})
