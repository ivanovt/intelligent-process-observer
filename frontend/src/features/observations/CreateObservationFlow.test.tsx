...
const generatedUuid='a1b2c3d4-e5f6-4a1b-8c9d-001122334455'
const generatedId=(name:string,kind:ObservationChildKind)=>generateObservationChildId(name,kind,[],generatedUuid)
async function createAlert(user:ReturnType<typeof userEvent.setup>, values:AlertValues={id:'release_alerts',name:'Release alerts',query:' project = REL  AND status != Done '}) {
  ...
  await user.type(screen.getByLabelText(/^Name\b/),values.name)
...
describe('Observation create routes',()=>{
  ...
  it('keeps the route-scoped draft through Alert edits, replaces by opaque key in order, and applies with zero writes',async()=>{
    ...
    await user.click(cards()[0].querySelector('a')!);await user.clear(screen.getByLabelText(/^Name\b/));await user.type(screen.getByLabelText(/^Name\b/),'Updated first');await user.click(screen.getByText('Apply changes'))
  ...
  it('cancels and browser-back editor changes without mutating the aggregate draft',async()=>{
    ...
    await user.click(screen.getByText('Edit'));await user.clear(screen.getByLabelText(/^Name\b/));await user.type(screen.getByLabelText(/^Name\b/),'Discarded');await user.click(screen.getByText('Cancel'))
    ...
    await user.click(screen.getByText('Edit'));await user.clear(screen.getByLabelText(/^Name\b/));await user.type(screen.getByLabelText(/^Name\b/),'Also discarded');window.history.back()
  ...
  it('posts one exact Metric-only aggregate, retains a selected non-default source through draft and failure, and succeeds on retry',async()=>{const user=userEvent.setup();let postCount=0;const fetchMock=vi.fn().mockImplementation((url:string,init?:RequestInit)=>{if(url==='/api/v1/observation-definition-capabilities')return Promise.resolve(response(metricCapabilities));if(url==='/api/v1/observations'&&init?.method==='POST'){postCount+=1;return Promise.resolve(postCount===1?response({code:'validation_error',message:'Correct Metric query',field:'lenses.0.query'},422):response(metricCreated,201))}return Promise.resolve(response(metricCreated))});vi.stubGlobal('fetch',fetchMock);renderAt('/observations/new');await user.type(await screen.findByLabelText('Name'),'Metric health');await user.type(screen.getByLabelText('Objective'),'Observe metrics');await createMetric(user,{id:'cpu',name:'CPU utilization',sourceId:'secondary'})
    ...
    const posts=fetchMock.mock.calls.filter(([url,init])=>url==='/api/v1/observations'&&(init as RequestInit).method==='POST');expect(posts).toHaveLength(2);expect(JSON.parse(String((posts[0][1] as RequestInit).body))).toEqual({name:'Metric health',description:null,objective:'Observe metrics',operational_context:null,lenses:[{id:generatedId('CPU utilization','metric'),name:'CPU utilization',description:null,type:'metric',metric_id:'node_cpu',adapter_type:'prometheus',source_id:'secondary',query:'rate_cpu_5m',unit:'%',analysis_objectives:['spike','drift'],reference_periods:['1d','7d']}],alert_lenses:[],relationships:[]});expect(posts.map(([,init])=>JSON.parse(String((init as RequestInit).body)).lenses[0].source_id)).toEqual(['secondary','secondary']);expect(JSON.stringify((posts[0][1] as RequestInit).body)).not.toMatch(/clientKey|history|preflight|tool|free-text/i);expect(fetchMock.mock.calls.filter(([url])=>String(url).includes('preflight')||String(url).includes('/lenses/'))).toEqual([])
  })
  it('preserves Metric order and opaque edit keys across edit, Cancel, and browser back with no write beyond capability GET',async()=>{const user=userEvent.setup(),fetchMock=vi.fn().mockImplementation(()=>Promise.resolve(response(metricCapabilities)));vi.stubGlobal('fetch',fetchMock);renderAt('/observations/new');await createMetric(user,{id:'first',name:'First metric'});await createMetric(user,{id:'second',name:'Second metric'});const list=()=>screen.getByRole('heading',{name:'Metric lenses'}).parentElement!,cards=()=>Array.from(list().querySelectorAll('p')).filter(card=>card.querySelector('a')),hrefs=()=>cards().map(card=>card.querySelector('a')?.getAttribute('href')),names=()=>cards().map(card=>card.childNodes[0].textContent?.trim());const initialHrefs=hrefs();expect(names()).toEqual(['First metric','Second metric']);await user.click(cards()[0].querySelector('a')!);await user.clear(screen.getByLabelText(/^Name\b/));await user.type(screen.getByLabelText(/^Name\b/),'Updated first');await user.click(screen.getByText('Apply changes'));expect(names()).toEqual(['Updated first','Second metric']);expect(hrefs()[0]).toBe(initialHrefs[0]);expect(hrefs()[1]).toBe(initialHrefs[1]);await user.click(cards()[0].querySelector('a')!);await user.clear(screen.getByLabelText(/^Name\b/));await user.type(screen.getByLabelText(/^Name\b/),'Discarded');await user.click(screen.getByText('Cancel'));expect(names()).toEqual(['Updated first','Second metric']);cleanup();window.history.replaceState({},'', '/observations/new');renderBrowserAt('/observations/new');await createMetric(user,{id:'back_first',name:'Back first'});const backCard=cards()[0],backHref=backCard.querySelector('a')?.getAttribute('href');await user.click(backCard.querySelector('a')!);await user.clear(screen.getByLabelText(/^Name\b/));await user.type(screen.getByLabelText(/^Name\b/),'Back discarded');window.history.back();expect(await screen.findByRole('heading',{name:'Create Observation'})).toBeTruthy();expect(names()).toEqual(['Back first']);expect(hrefs()[0]).toBe(backHref);expect(fetchMock.mock.calls.filter(([,init])=>(init as RequestInit | undefined)?.method==='POST')).toEqual([])
  })
