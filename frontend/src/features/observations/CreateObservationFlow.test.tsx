...
async function createMetric(user:ReturnType<typeof userEvent.setup>, values={id:'cpu',name:'CPU utilization'}) {await user.click(screen.getByText('Add Metric Lens'));await screen.findByRole('option',{name:'Primary Prometheus'});await user.type(screen.getByLabelText('Name'),values.name);await user.type(screen.getByLabelText('Metric ID'),'node_cpu');await user.type(screen.getByLabelText('Unit'),'%');await user.selectOptions(screen.getByLabelText('Metric source'),'primary');await user.type(screen.getByLabelText('Provider query'),'rate_cpu_5m');await user.click(screen.getByLabelText('spike'));await user.click(screen.getByLabelText('drift'));await user.click(screen.getByText('Add reference period'));await user.type(screen.getByLabelText('Reference period 1'),'1d');await user.click(screen.getByText('Add reference period'));await user.type(screen.getByLabelText('Reference period 2'),'7d');await user.click(screen.getByText('Apply changes'))}
