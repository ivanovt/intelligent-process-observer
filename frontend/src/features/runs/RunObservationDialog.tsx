...
function TimeRangeChooser({ input, preview, onChange }: { input: TimeRangeInput; preview: ReturnType<typeof resolveTimeRange>; onChange: (input: TimeRangeInput) => void }) {
  ...
  const isExpressions = input.kind === 'expressions'
  return <fieldset className="grid gap-4 rounded-lg border border-[var(--color-border)] p-4"><legend className="px-1 text-sm font-semibold">Analysis time range</legend><div className="flex flex-wrap gap-4 text-sm"><label className="flex items-center gap-2"><input checked={isPreset} name="range-mode" type="radio" onChange={() => onChange({ kind: 'preset', preset: '15m' })} />Relative range</label><label className="flex items-center gap-2"><input checked={isExpressions} name="range-mode" type="radio" onChange={() => onChange({ kind: 'expressions', from: 'now-15m', to: 'now' })} />Expressions</label><label className="flex items-center gap-2"><input checked={input.kind === 'absolute'} name="range-mode" type="radio" onChange={() => onChange({ kind: 'absolute', from: '', to: '' })} />Absolute UTC</label></div>{isPreset ? <Field label="Relative range"><Select aria-label="Relative analysis range" value={input.preset} onChange={(event) => onChange({ kind: 'preset', preset: event.target.value as RelativePresetId })}>{relativePresets.map((preset) => <option key={preset.id} value={preset.id}>{preset.label}</option>)}</Select></Field> : isExpressions ? <div className="grid gap-4 sm:grid-cols-2"><Field label="From" description="Supported: now, now-15m, now-1h"><Input aria-label="From expression" value={input.from} onChange={(event) => onChange({ kind: 'expressions', from: event.target.value, to: input.to })} /></Field><Field label="To" description="Supported: now, now-15m, now-1h"><Input aria-label="To expression" value={input.to} onChange={(event) => onChange({ kind: 'expressions', from: input.from, to: event.target.value })} /></Field></div> : <div className="grid gap-4 sm:grid-cols-2"><Field label="From (UTC)" description="Enter an exact UTC date and time, including seconds."><Input step="1" type="datetime-local" value={input.from} onChange={(event) => onChange({ kind: 'absolute', from: event.target.value, to: input.to })} /></Field><Field label="To (UTC)" description="Enter an exact UTC date and time, including seconds."><Input step="1" type="datetime-local" value={input.to} onChange={(event) => onChange({ kind: 'absolute', from: input.from, to: event.target.value })} /></Field></div>}{preview.ok ? <p aria-live="polite" className="text-sm text-[var(--color-text-secondary)]">Resolved UTC window: {preview.value.from} to {preview.value.to}</p> : <InlineNotice tone="error">{preview.error}</InlineNotice>}</fieldset>
}
