...
describe('RunsPage', () => {
  ...
  it('navigates an Open action to the registered run detail route', async () => {
    ...
    renderAt()
    await screen.findByLabelText('Observation runs')
    const open = screen.getByRole('link', { name: 'Open run run-newest-1234' })
    expect(open.className).toContain('text-[var(--color-primary)]')
    expect(open.querySelector('.lucide-arrow-right')).toBeTruthy()
